...
#'    A50_mat = 8, Rec_stochasticity = FALSE, Rho_R = 0,
...
stable_AD <- function(Rec_age, Max_age, W, R0, Mat, H, B0, Sigma_R, Fb, S, M,
  ...
  for (t in (Rec_age + 1):(eq_time - 1)) {
    ...
    # recruitment
    R <- recruitment(t, cr = 1, fdr = 1, SSB = SSB2, A, R0, H, B0,
                     Eps = Eps2, Sigma_R, Rec_age, Recruitment_mode, LDP = 0,
                     Rec_stochasticity = FALSE)
  ...
  # plot(1:(eq_time - 1), N2[1, 1, 1:(eq_time - 1), 1, 1], type = 'l',
  #      ylim = c(0, 5e3), col = 'green')
  # for (x in 2:(num - 1)) {
  #   lines(1:(eq_time - 1), N2[x, 1, 1:(eq_time - 1), 1, 1], col = 'red')
  # }
  # lines(1:(eq_time - 1), N2[num, 1, 1:(eq_time - 1), 1, 1], col = 'blue')
