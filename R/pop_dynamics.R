...
#'    Abundance, Fishing = TRUE, M = 0.17, R, Ind_sampled = 'all')
pop_dynamics <- function(t, cr, fdr, Rec_age, Max_age, SSB, N, W, Mat, A = 5,
                         Fb, E, S, FM, A50_mat, Biomass, Abundance, Fishing = T,
                         M, R, Ind_sampled = 'all') {
  ...
  for (i in 2:(num - 1)) {
    N[i, , t, cr, fdr] <- N[i - 1, , t - 1, cr, fdr] * exp(-1 * (FM[i - 1, , t - 1, cr, fdr] + M))
  }

  # Final age bin
  N[num, , t, cr, fdr] <- N[num - 1, , t - 1, cr, fdr] * exp(-1 * (FM[num - 1, , t - 1, cr, fdr] + M)) +
