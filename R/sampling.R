...
#' sampling(t = 51, cr = 1, fdr = 1, Delta = 1.6, Gamma = 31.6, Abundance,
#'    Transects, X = 15.42, Count, NuS, A, Ind_sampled = 'all')
sampling <- function(t, cr, fdr, Delta, Gamma, Abundance, Transects = 24,
  ...
  if (fdr > dim(Abundance)[4] || fdr > dim(Count)[7]) {
  ...
  All <- Gamma*Abundance[, t - 1, cr, fdr, 1]*exp(nus)
