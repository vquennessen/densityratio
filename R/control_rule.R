...
control_rule <- function(t, cr, nm, fdr, A = 5, E, Count, Time1 = 50,
  ...
  if(dim(E)[5] != dim(Count)[7]) {
    stop('E or Count has an incorrect number of final density ratios.')}
  ...
  if (BM == FALSE) {
    ...
    BM_Transects <- ifelse(Sampling_Error == TRUE, 1000, Transects)
