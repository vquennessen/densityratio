...
A = 5; MPA = 3
Time1 = 50; Time2 = 50
M_Error = 0; Sampling_Error = TRUE; Rec_stochasticity = FALSE
...
Output.Biomass = FALSE; Output.SSB = TRUE
Output.Yield = TRUE; Output.Effort = TRUE; Output.Density.Ratio = FALSE
```
  ...
  for (am in 1:length(Adult_M)) {
    for (a in 1:length(Allocations)) {
      ...
      output <- base_model(Species, R0, A, MPA, Time1, Time2, 
                           ...
                           Rec_stochasticity, Surveys, Fishery_management, 
                           Fishing, Transects, Adult_movement, Plotting, 
                           Final_DRs, Years_sampled, Areas_sampled, Ind_sampled, 
                           Floor_DR, Allocation, BM, LDP, Control_rules, 
...
# plot Figure 4a
ggplot(data, aes(x = Index, y = SSB, fill = as.factor(Decade))) + 
  geom_bar(stat = 'identity', position = 'dodge') +
  geom_point(aes(x = Index, y = yield)) +
  labs(fill = 'Decade')
```
